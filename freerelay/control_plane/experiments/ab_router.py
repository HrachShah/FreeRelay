...
class ExperimentManager:
    ...
    async def record_outcome(
        ...
        try:
            # Use a single pipeline for all writes, then read the updated totals
            # for the rollback check to avoid a stale-read race between hget and hset.
            pipe = self._redis.pipeline()
            pipe.hincrby(m_key, "total_requests", 1)
            if success:
                pipe.hincrby(m_key, "success_count", 1)
            # Read current float accumulators before incrementing them
            ...
            if (
                ...
                # Read total_requests from Redis directly so we see the already-
                # incremented value from this same pipeline rather than the stale
                # count that existed before we sent the hincrby.
                new_total = await self._redis.hget(m_key, "total_requests")
                total_requests = int(new_total) if new_total else 0
